...
Persistence: (1) edits save to `localStorage` → instant, but only in the editor's own browser; (2) "Yayınla" downloads a new `assets/fenix.overrides.js` — replace that file and re-deploy to make edits live for **everyone**; (3) OPTIONAL backend (`api/overrides.js` read + `api/publish.js` write, Vercel KV, zero deps) — the admin's "Canlıya Yayınla" button POSTs to `/api/publish` (auth via `ADMIN_TOKEN` env) so edits go live for everyone with **no re-deploy**. Storefront pages load `/api/overrides.js` right after the static `fenix.overrides.js` (backend wins when present; static file is the fallback). Setup guide: `BACKEND.md`.
